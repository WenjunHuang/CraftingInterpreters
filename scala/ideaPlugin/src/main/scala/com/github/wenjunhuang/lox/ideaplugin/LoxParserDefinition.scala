...
class LoxParserDefinition extends ParserDefinition:
  ...
  override def createElement(node: ASTNode): PsiElement = LoxElementTypes.Factory.createElement(node)

  override def createFile(viewProvider: FileViewProvider): PsiFile = LoxFile(viewProvider)
end LoxParserDefinition

object LoxParserDefinition:
  val FILE: IFileElementType = IFileElementType(Lox)
